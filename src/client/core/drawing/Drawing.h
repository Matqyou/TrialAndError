...
public:
	DrawCommand(Uint32 init_num_vertices, Uint32 init_num_indices, SDL_GPUTexture *init_texture = nullptr);
	~DrawCommand();
...
//	Uint32 SizeGPUIndices() { return size_indices; }
	...
	void SetTexture(SDL_GPUTexture *new_texture) { texture = new_texture; }
	void SetTranslation(const Vec2f& new_translation) { translation = new_translation; }
...
public:
	Quad(DrawCommand& draw_command, const Rect4f& start_rect, const SDL_Color& start_color = { 255, 255, 255, 150 });
	...
	void UpdateRect(const Rect4f& new_rect);
};
...
//	void SetColor(SDL_Color color) { SetColor(color.r, color.g, color.b, color.a); }
	...
	// Drawing
	void DrawFilledRect(const SDL_FRect& rect, uint8_t r, uint8_t g, uint8_t b, uint8_t a);
...
//	void UpdateWindow();  // Pushes the latest state of the pixels to window
	...
	void SetPipeline2D();
	void ShaderTick2D();
	void ShaderParams2D(float rotation, const Vec2f& translation, bool use_texture);
