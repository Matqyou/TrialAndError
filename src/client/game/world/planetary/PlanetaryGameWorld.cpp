...
Vec2f PlanetaryWorld::GetSurfaceNormal(const Vec2f& world_pos) const
{
	// Surface normal points away from planet center
	return world_pos.NormalizeF();
}

void PlanetaryWorld::RenderPlanet()
{
	RenderPlanetCore(Application.GetRenderer(), Application.GetWidth(), Application.GetHeight());
}
...
void PlanetaryWorld::RenderPlanetCore(SDL_Renderer *renderer, int screen_width, int screen_height)
{
	...
	for (int ring = 0; ring <= rings; ++ring)
	{
		...
		for (int seg = 0; seg <= segments; ++seg)
		{
			...
			float brightness = static_cast<float>(light_intensity);
...
void PlanetaryWorld::RenderAtmosphere(SDL_Renderer *renderer, int screen_width, int screen_height)
{
	...
	Vec2f planet_screen_pos = Vec2f(PlanetaryUtils::WorldToScreen(Vec2f(0, 0), m_CameraPos, screen_width, screen_height, m_CameraZoom));
	...
	for (int layer = 0; layer < 5; layer++)
	{
		...
		for (int angle = 0; angle < 360; angle += 5)
		{
			...
			float x = (planet_screen_pos.x + cosf(rad) * layer_radius);
			float y = (planet_screen_pos.y + sinf(rad) * layer_radius);
...
void PlanetaryWorld::RenderBackgroundLayers(SDL_Renderer *renderer, int screen_width, int screen_height)
{
	...
	{
		float parallax_factor = 1.0f / m_LayerDistances[i];
		Vec2f layer_offset = m_CameraPos * parallax_factor;

		// Tile the background layer
		float tile_width = m_BackgroundLayers[i]->GetWidth();
		float tile_height = m_BackgroundLayers[i]->GetHeight();

		int start_x = (int)(layer_offset.x / tile_width) - 1;
		int start_y = (int)(layer_offset.y / tile_height) - 1;
		int end_x = start_x + (screen_width / static_cast<int>(tile_width)) + 3;
		int end_y = start_y + (screen_height / static_cast<int>(tile_height)) + 3;
		...
		for (int ty = start_y; ty < end_y; ty++)
		{
			...
			{
				SDL_FRect dst_rect = {
					static_cast<float>(tx) * tile_width - layer_offset.x,
					static_cast<float>(ty) * tile_height - layer_offset.y,
					tile_width, tile_height
				};
				SDL_RenderTexture(renderer, m_BackgroundLayers[i]->SDLTexture(), nullptr, &dst_rect);
			}
		}
	}
}

void PlanetaryWorld::UpdateCamera(const Vec2f& target_pos, float zoom_level)
{
	m_CameraPos = target_pos;
	m_CameraZoom = zoom_level;
}

PlanetaryCoords PlanetaryWorld::WorldToPlanetary(const Vec2f& world_pos) const
{
	return PlanetaryCoords::FromCartesian(world_pos, m_Config.radius);
}

Vec2f PlanetaryWorld::PlanetaryToWorld(const PlanetaryCoords& coords) const
{
	return coords.ToCartesian();
}

float PlanetaryWorld::GetDistanceFromSurface(const Vec2f& world_pos) const
{
	return world_pos.LengthF() - m_Config.radius;
}

PlanetaryGameWorld::PlanetaryGameWorld(int width, int height)
	: GameWorld(width, height), m_CurrentPlanet(nullptr), m_CurrentPlanetIndex(0)
{
	// Initialize with a default planet configuration
	PlanetConfig defaultPlanet;
	defaultPlanet.radius = static_cast<double>(width * height) / 2.0;
...
//	PlanetaryCharacter *player = dynamic_cast<PlanetaryCharacter *>(FirstCharacter());
	...
	Vec2f playerWorld = playerCoords.ToCartesian();

	Vec2f lookDir = player->GetDirectionalCore().direction;

	const double radius = m_CurrentPlanet->GetConfig().radius;
	Vec2f targetCam = {
		playerWorld.x + lookDir.x,
		(playerWorld.y + lookDir.y) * 0.75f
	};

//	Drawing* drawing = Application.GetDrawing();
	auto camera = GameReference.GetCamera();
	Vec2f camera_pos = camera.GetPos();
...
//	double oldCamY = render->GetCameraY();
	...
	Vec2f new_camera_pos = camera_pos + (targetCam - camera_pos) * followSpeed;
...
void PlanetaryGameWorld::RenderPlanetaryBackground()
{
	...
	Drawing *drawing = Application.GetDrawing();
	drawing->SetColor(5, 5, 20, 255); // Deep space color
	drawing->Clear();

	// Render the planet and its atmosphere
	m_CurrentPlanet->RenderPlanet();
}

void PlanetaryGameWorld::TickCamera(double seconds_elapsed)
{
	TickPlanetaryCamera();
}

void PlanetaryGameWorld::TickBackground(double seconds_elapsed)
{
	if (!m_CurrentPlanet)
	{
		// Fallback to regular background if no planet
		GameWorld::TickBackground(seconds_elapsed);
		return;
	}
	...
	for (int i = 0; i < 2; i++) // Fewer particles for performance
	{
		...
		Vec2f screen_pos = camera.CameraToScreenPoint(random_position);
...
//														 drawing->GetZoom());
	...
	Vec2f mouse_position = Application.GetMousePosition();
...
//	auto MousePositionD = Vec2d(MousePosition.x, MousePosition.y);
	...
	for (int i = m_Stars.size() - 1; i >= 0; --i)
	{
		...
		auto direction = position - mouse_position;
		...
		Vec2f planet_screen_center = camera.CameraToScreenPoint(Vec2f(0.0f, 0.0f));
...
//																   drawing->GetZoom());
		...
		Vec2f to_planet = planet_screen_center - position;
		double planet_distance = to_planet.Length();
		if (planet_distance > 0)
		{
			// Weak orbital influence
			Vec2f orbital_force = Vec2f(-to_planet.y, to_planet.x).NormalizeF() * 0.01f;
			velocity += orbital_force;
		}

		velocity *= 0.995f; // Slight drag
...
void PlanetaryGameWorld::Tick(double seconds_elapsed)
{
	...
	TickBackground(seconds_elapsed);
	TickSpawner(seconds_elapsed);
	TickEntities(seconds_elapsed);
	TickDestroy(seconds_elapsed);
	m_Particles->Tick();
	TickCamera(seconds_elapsed);
...
void PlanetaryGameWorld::Draw()
{
	...
	drawing->SetColor(200, 200, 255, 255); // Slightly blue-white for space
	for (int i = m_Stars.size() - 1; i >= 0; --i)
	{
		auto& [position, velocity, duration] = m_Stars[i];
		...
		for (int j = 0; j < size; j++)
		{
			for (int k = 0; k < size; k++)
			{
				...
				{ // Twinkle pattern
					SDL_RenderPoint(renderer, draw_x, draw_y);
				}
			}
		}
	}
	...
	else
	{
		...
		float pw = (float)Application.GetWidth() * 0.6f;
		float ph = (float)Application.GetHeight() * 0.6f;
		float px = ((float)Application.GetWidth() - pw) / 2.0f;
		float py = ((float)Application.GetHeight() - ph) / 2.0f;
		...
		float tw = tTex->GetWidth() * 3;
		float th = tTex->GetHeight() * 3;
		SDL_FRect titleRect = { px + (pw - tw) / 2, py + 20, tw, th };
		...
		float buttonY = py + ph - buttonHeight - 40;
